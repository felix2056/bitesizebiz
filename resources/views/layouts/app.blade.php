...
<body class="page-template page-template-page-blog page-template-page-blog-php page page-id-126 theme-xoxo woocommerce-no-js">
                        ...
                        <div class="search_result">
                            <div class="filterbox">
                                <div class="filter title_filter">
                                    <label>
                                        <input type="checkbox" name="titleFilter" value="1">
                                ...
                                <div class="filter post_filter">
                                    <label>
                                        <input type="checkbox" name="postFilter" value="1">
                                        <span class="icon"><img class="fn__svg " src="/images/svg/checked.svg" alt="svg"></span>
    ...
    <script>
        ...
        const supabaseUrl = 'https://lvenlmdooekehteexjev.supabase.co'
        const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx2ZW5sbWRvb2VrZWh0ZWV4amV2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzM1NTU5MTksImV4cCI6MjA0OTEzMTkxOX0._laS01Ku0wufOIxZ2DbY6TeDt435rSOKQgYEYEgdSmc"
        ...
        async function fetchCategories() {
            try {
                ...
                // Update the categories array with fetched data
                categories = data;
        ...
        async function fetchPosts() {
            try {
                ...
                // Call your render functions after data is loaded
                populateSliderBlogPosts();
                populateBlogPosts();
                populateTopBottomTrendingPost();
        ...
        // Function to populate the slider with blog posts
        function populateSliderBlogPosts() {
            const container = document.getElementById('slider-blog-posts-container');

            // Clear any existing content
            container.innerHTML = '';

            // Generate slides from data array
            posts.forEach(post => {
                container.innerHTML += generateSlideHTML(post);
            });

            // Load background images
            document.querySelectorAll('.abs_img[data-bg-img]').forEach(elem => {
                elem.style.backgroundImage = `url(${elem.getAttribute('data-bg-img')})`;
            });

            // If using Swiper and it's already initialized, update it
        ...
        function populateBlogPosts() {
            ...
            // Get top 3 posts by views for trending section
            const trendingPosts = [...posts]
                .sort((a, b) => b.views - a.views)
                .slice(0, 3);

            // Insert regular blog posts with trending section after the 5th post
            posts.forEach((post, index) => {
                if (post.type === "link") {
                    container.innerHTML += generateLinkHTML(post);
                } else if (post.type === "video") {
                    container.innerHTML += generateVideoHTML(post);
                } else {
                    container.innerHTML += generateBlogPostHTML(post);
                }

                // Insert trending section after the 5th post
                if (index === 4) {
                    container.innerHTML += generateTrendingHTML(trendingPosts);
                }
            });
        }
        ...
        function populateCategoriesList() {
            ...
            categories.forEach(category => {
                const category_count = posts.filter(post => post.category === category.name).length;
                categoriesList.innerHTML += `
                    <li>
                        <div class="category__item">
                            ...
                            <span class="cat_title">
                                ...
                                <span class="count">${category_count}</span>
                            </span>
                        </div>
                    </li>
                `;
        ...
        // Function to populate the top and bottom trending posts
        function populateTopBottomTrendingPost() {
            const top_container = document.getElementById('top-trending-post-container');
            const bottom_container = document.getElementById('bottom-trending-post-container');
            ...
            // Get top 3 posts by views for trending section
            const topTrendingPost = [...posts]
                .sort((a, b) => b.views - a.views)
                .slice(0, 1);

            const bottomTrendingPosts = [...posts]
                .sort((a, b) => b.views - a.views)
                .slice(1, 3);

            top_container.innerHTML += generateTopTrendingHTML(topTrendingPost);
            bottom_container.innerHTML += generateBottomTrendingHTML(bottomTrendingPosts);
        }


        // Function to generate the HTML for each slide
        function generateSlideHTML(post) {
            if (!post) return '';

            const categoryName = post.categories ? .name || 'Uncategorized';
            const categoryLink = post.categories ? .link || '#';
                        ...
                        <div class="fn__meta">
                            <p>
                                ...
                                <span class="meta_item meta_category">
                                    <a class="fn_last_category" href="${categoryLink}">${categoryName}</a>
                                </span>
        ...
        // Function to generate the HTML for each blog post
        ...
        // Function to generate the link html
        function generateLinkHTML(post) {
            if (!post) return '';

            return `
                <li class="post-103 post type-post status-publish format-link has-post-thumbnail hentry category-travel tag-blog tag-image tag-travel tag-writing post_format-post-format-link post_item" id="post-103">
                    <div class="blog__item blog__item_masonry" data-short-preview="">
        ...
        // Function to generate the HTML for each video post
        function generateVideoHTML(post) {
            if (!post) return '';
                    ...
                    <div class="bottom_holder">
                        ...
                        <div class="blog__image">
                            <iframe 
                                width="100%" 
                                height="352" 
                                src="https://www.youtube.com/embed/${post.video_url.split('v=')[1]}"
        ...
        // Function to generate the trending posts HTML
        function generateTrendingHTML(trendingPosts) {
        ...
        // Function to generate the top trending post HTML
        function generateTopTrendingHTML(trendingPosts) {
            if (!trendingPosts) return '';

            return `
            <div class="top_article">
                <div class="item_img">
                    <a class="full_link" href="${trendingPosts[0].link}"></a>
                    <img src="${trendingPosts[0].image_url}">
                </div>

                <h3 class="fn_title">
                    <a href="${trendingPosts[0].link}">${trendingPosts[0].title}</a>
                </h3>
        ...
        // Function to generate the bottom trending post HTML
        function generateBottomTrendingHTML(trendingPosts) {
            if (!trendingPosts) return '';
        ...
        // Function to format views in K format
